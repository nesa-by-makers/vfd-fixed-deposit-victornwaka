	...
	<body><?php  session_start(); if(!isset($_SESSION["reference"])){
		...
		<!-- Main -->
          ...
          <h3>CUSTOMER ACCOUNTS IN VFD MICROFINANCE BANK</h3>
           ...
           $query1 = "select SN, fullname, emailadd, position from employee_register where position != 'CEO';";
               $results1 = $connection->query($query1);
                   while ($row1 = $results1->fetch_assoc()){
                       
                       echo  "<td>".$row1["SN"]."</td><td>".$row1["fullname"]."</td><td>".$row1["emailadd"]. "</td> <td>".$row1["position"]."</td></tr>"; 
                   }
