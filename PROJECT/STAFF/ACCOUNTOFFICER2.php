	...
	<body><?php  session_start(); if(!isset($_SESSION["reference"])){
		...
		<!-- Main -->
			...
			<table>
				...
				$query1 = "select * from personal_details where reference = '".$_SESSION["reference"]."' and verify IS NULL;";
					$results1 = $connection->query($query1);
						while ($row1 = $results1->fetch_assoc()){
							...
							echo "<td>".$row3["name_of_bank"]."</td><td>".$row3["acc_name"]. "</td> <td>".$row3["acc_number"]."</td><td><form action ='redirectverify.php' method ='POST' ><input type='hidden' value='".$row1["SN"]."' name='input2'/><button id ='".$row1["SN"]."' >VERIFY</button></form></td></tr>";
